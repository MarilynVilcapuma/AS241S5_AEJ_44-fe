    ...
    <>
      ...
      <div className="page-header">
        ...
        <button
          ...
          style={{
            ...
            boxShadow: '0 4px 14px rgba(67,56,202,0.4)', transition: 'transform 0.15s',
          }}
      ...
      <div className="stats-row">
        ...
        <div className="stat-card active" onClick={() => setVerdictFilter('HUMAN')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info">
            <span className="stat-value">{humanCount}</span>
            <span className="stat-label">Humano</span>
          </div>
        </div>
        <div className="stat-card inactive" onClick={() => setVerdictFilter('AI')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info">
            ...
            <span className="stat-label">IA detectada</span>
          </div>
        </div>
      </div>
      ...
      <Card>
        ...
        <div className="filter-bar">
          <div className="filter-search">
            ...
            <input
              ...
              placeholder="Buscar por texto analizado..."
            />
            {searchText && (
          ...
          <div className="filter-pills">
            <button className={`filter-pill pill-all ${statusFilter === 'all' ? 'active' : ''}`} onClick={() => setStatusFilter('all')}>
              Todos <span className="pill-badge">{totalCount}</span>
            </button>
        ...
        <div className="filter-bar-secondary">
          <span>Veredicto:</span>
          <button className={`filter-pill-sm ${verdictFilter === 'all' ? 'active-all' : ''}`} onClick={() => setVerdictFilter('all')}>
            Todos
          </button>
          <button className={`filter-pill-sm ${verdictFilter === 'HUMAN' ? 'active-human' : ''}`} onClick={() => setVerdictFilter('HUMAN')}>
            <i className="pi pi-user" style={{ fontSize: '0.72rem', marginRight: '4px' }} />
            Humano ({humanCount})
          </button>
          <button className={`filter-pill-sm ${verdictFilter === 'AI' ? 'active-ai' : ''}`} onClick={() => setVerdictFilter('AI')}>
            <i className="pi pi-android" style={{ fontSize: '0.72rem', marginRight: '4px' }} />
            IA ({aiCount})
          </button>
          <button className={`filter-pill-sm ${verdictFilter === 'MIXED' ? 'active-mixed' : ''}`} onClick={() => setVerdictFilter('MIXED')}>
            <i className="pi pi-chart-pie" style={{ fontSize: '0.72rem', marginRight: '4px' }} />
            Mixto ({mixedCount})
          </button>
        </div>
        ...
        <div className="table-toolbar">
          ...
          <span className="table-toolbar-count">
            {hasFilters
              ? `${filtered.length} de ${totalCount} elementos`
      ...
      <DetectorEditDialog
        ...
        visible={!!editDetection}
        onHide={() => setEditDetection(null)}
