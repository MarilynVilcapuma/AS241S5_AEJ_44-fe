    ...
    <>
      ...
      <Card>
        ...
        <div className="filter-bar">
          ...
          <div className="filter-pills">
            <button className={`filter-pill pill-all ${statusFilter === 'all' ? 'active' : ''}`} onClick={() => setStatusFilter('all')}>
              Todos <span className="pill-badge">{media.length}</span>
            </button>
        ...
        <div className="filter-bar-secondary">
          <span>Tipo:</span>
          <button className={`filter-pill-sm ${typeFilter === 'all' ? 'active-all' : ''}`} onClick={() => setTypeFilter('all')}>
            Todos
          </button>
          <button className={`filter-pill-sm ${typeFilter === 'IMAGE' ? 'active-img' : ''}`} onClick={() => setTypeFilter('IMAGE')}>
            <i className="pi pi-image" style={{ fontSize: '0.72rem', marginRight: '4px' }} />
            Imágenes ({imageCount})
          </button>
          <button className={`filter-pill-sm ${typeFilter === 'VIDEO' ? 'active-video' : ''}`} onClick={() => setTypeFilter('VIDEO')}>
            <i className="pi pi-video" style={{ fontSize: '0.72rem', marginRight: '4px' }} />
            Videos ({videoCount})
          </button>
        </div>
        ...
        <div className="table-toolbar">
          ...
          <span className="table-toolbar-count">
            {hasFilters
              ? `${filteredMedia.length} de ${media.length} elementos`
              : `${media.length} elemento${media.length !== 1 ? 's' : ''}`
            }
          </span>
        </div>
