...
export default function DetectorTable({ detections, loading, onEdit, onDeactivate, onRestore }: DetectorTableProps) {
  ...
  const inputTextTemplate = (d: Detection) => (
    <div title={d.inputText} style={{ maxWidth: '280px' }}>
      <div style={{ fontSize: '0.85rem', color: '#334155', lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {d.inputText}
      </div>
  ...
  const actionBtn = (icon: string, hoverColor: string, hoverBg: string, iconColor: string, title: string, onClick: () => void) => (
    <button title={title} onClick={onClick}
      ...
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'transparent'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = iconColor }}
      style={{ width: 34, height: 34, borderRadius: '50%', border: '2px solid transparent', background: 'transparent', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: iconColor, transition: 'all 0.18s', outline: 'none', flexShrink: 0 }}
    >
    ...
    <DataTable
      ...
      emptyMessage={
        <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <div style={{ color: '#94a3b8', fontSize: '0.85rem', marginTop: '0.25rem' }}>Analiza tu primer texto con el botón "Nueva Detección"</div>
        </div>
      }
