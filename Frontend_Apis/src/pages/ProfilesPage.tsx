...
export default function ProfilesPage() {
  ...
  const filteredProfiles = profiles.filter(p => {
    const matchSearch = p.username.toLowerCase().includes(searchText.toLowerCase()) || (p.fullName ?? '').toLowerCase().includes(searchText.toLowerCase())
    ...
    <>
      <div className="page-header">
        ...
        <button onClick={() => setShowForm(true)}
          ...
          <i className="pi pi-plus" style={{ fontSize: '0.9rem' }} /> Nuevo Perfil
        </button>
      </div>
      ...
      <div className="stats-row">
        <div className="stat-card total" onClick={() => { setStatusFilter('all'); setSearchText('') }} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info"><span className="stat-value">{profiles.length}</span><span className="stat-label">Total perfiles</span></div>
        </div>
        <div className="stat-card active" onClick={() => setStatusFilter('active')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info"><span className="stat-value">{activeCount}</span><span className="stat-label">Activos</span></div>
        </div>
        <div className="stat-card inactive" onClick={() => setStatusFilter('inactive')} style={{ cursor: 'pointer' }}>
          ...
          <div className="stat-info"><span className="stat-value">{inactiveCount}</span><span className="stat-label">Inactivos</span></div>
        </div>
      </div>
      ...
      <Card>
        <div className="filter-bar">
          <div className="filter-search">
            ...
            <input value={searchText} onChange={e => setSearchText(e.target.value)} placeholder="Buscar por usuario o nombre..." />
          ...
          <div className="filter-pills">
            <button className={`filter-pill pill-all ${statusFilter === 'all' ? 'active' : ''}`} onClick={() => setStatusFilter('all')}>Todos <span className="pill-badge">{profiles.length}</span></button>
        ...
        <div className="table-toolbar">
          <span className="table-toolbar-title"><i className="pi pi-list" style={{ color: '#6366f1' }} /> Lista de perfiles</span>
          <span className="table-toolbar-count">{hasFilters ? `${filteredProfiles.length} de ${profiles.length} perfiles` : `${profiles.length} perfil${profiles.length !== 1 ? 'es' : ''}`}</span>
        </div>

        <ProfileTable profiles={filteredProfiles} loading={loading} onEdit={profile => setEditProfile(profile)} onDeactivate={handleDeactivate} onActivate={handleActivate} />
      </Card>

      <ProfileForm visible={showForm} onHide={() => setShowForm(false)} onSubmit={handleCreate} loading={loading} />
      <ProfileEditDialog profile={editProfile} visible={!!editProfile} onHide={() => setEditProfile(null)} onUpdate={handleUpdate} loading={loading} />
    </>
  )
}
